...
namespace DicomReader.Avalonia.Models
{
    ...
    {
        public DicomQueryInputs(DicomRequestType requestType, DicomQueryRetrieveLevel retrieveLevel, string patientId, string studyInstanceUid,
            string accessionNumber, IList<DicomTagItem> requestedDicomTags, bool isPagedQuery, int? pageSize)
        {
            ...
            switch (requestType)
            {
                ...
                case DicomRequestType.Custom:
                    if (retrieveLevel == DicomQueryRetrieveLevel.Patient && patientId.IsNullOrEmpty())
                        throw new InvalidOperationException("Patient id required for retrieve level patient");

                    if (retrieveLevel == DicomQueryRetrieveLevel.Study && studyInstanceUid.IsNullOrEmpty() && accessionNumber.IsNullOrEmpty())
                        throw new InvalidOperationException("Retrieve level study requires a study instance uid or accession number");

                    if (retrieveLevel == DicomQueryRetrieveLevel.Series && studyInstanceUid.IsNullOrEmpty())
        ...
        private static bool ValidRetrieveLevel(DicomQueryRetrieveLevel retrieveLevel) =>
